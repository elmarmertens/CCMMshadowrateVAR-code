function Diagnostics(sqrtht,invA,PAI,PHI,N,K,M)

for i=1:N
display('--------------------------');
display(['volatility eq.' num2str(i)]);
PSRFi(i)=mean(psrf(sqrtht(:,:,i))); %#ok<AGROW>
display(['Average PSRF  ' num2str(PSRFi(i)) ]);
IFi(i,:)=mean(ineff(sqrtht(:,:,i))); %#ok<AGROW>
...
display('--------------------------');
temp=reshape(invA,M,N*N);
temp2=[]; for i=1:size(temp,2); if temp(1,i)==1 || temp(1,i)==0; else temp2=[temp2 temp(:,i)]; end; end %#ok<AGROW>
display(['invA Average PSRF   ' num2str(mean(psrf(temp2)))]);
display(['invA Average IF (4% 8% 15% tapering) ' num2str(mean(ineff(temp2)))]);

display('--------------------------');
display(['PHI Average PSRF ' num2str(mean(psrf(PHI)))]);
display(['PHI Average IF (4% 8% 15% tapering) ' num2str(mean(ineff(PHI)))]); 
