...
- doVARshadowrateBlockNonstructural.m:184
- doVARshadowrateNonstructural.m:184
...
The main computational routines last run on a **32-core Intel virtual client with 112 GB of RAM**. Computation took abut **[30] hours**. 
...
- Each `bot*m` file is a copy of a corresponding `go` file, but with presets that apply the model to a particular dataset. In a bash shell (or zshell), you can create this set of `bot` files, by calling the bash script [preparebatch.sh](preparebatch.sh) and pass the appropriate `go` file names as argument; for exammple `sh preparebatch.sh goVAR*.m` prepares `bot` files for all `go` files in the repo and prepares estimation of each model both with and without longer-run yields data (as needed in the paper). For example, to rerun all results with a different number of MCMC draws, change the value of `MCMCdraws` on line 19 of [preparebatch.sh](preparebatch.sh).
...
- In case of [goparbatch.sh](goparbatch.sh), the Matlab session opens a parallel pool. For example, the shell command `sh gobatch.sh goVAR.m goVARshadowrateBlockNonstructural.m goVARhybrid.m` will launch a command line session of Matlab, start a parallel pool, and then execute [goVAR.m](goVAR.m); once [goVAR.m](goVAR.m) has been executed, the Matlab sessions closes, a new one is reopened for execution of [goVARshadowrateBlockNonstructural.m](goVARshadowrateBlockNonstructural.m) etc. (The shell script supports as many command line arguments as supported by bash and has been written for use on macOS and Linux.) Alternatively, Matlab scripts can, of course, equally be called interactively on the Matlab GUI’s command line.
