%% general version VAR of <<Forecasting with Shadow-Rate VARs>>
...
modellabel = 'ELBshadowrateGeneralAR1SV';
...
[BETAmedian, BETAmean, BETAstdev] = deal(NaN(Nshadowrates, N-Nyields, Njumpoffs));
BETAquantiles = NaN(Nshadowrates, N-Nyields, Nquantiles, Njumpoffs);
...
if ~doLoMem
    ...
    drawsBETA      = NaN(MCMCdraws, Nshadowrates, N-Nyields, Njumpoffs);
...
parfor ndxT = 1 : Njumpoffs % parfor
    ...
    while ~mcmcOK
        try % catch crashes and continue
            ...
            [PAI_all, hRHO_all, hBAR_all, PHI_all, invA_all, sqrtht_all, shadowrate_all, missingrate_all, ...
                ...
                ydraws, yhat, ...
                shadowratedraws, shadowratehat, ...
                ] = mcmcVARshadowrateGeneralAR1SV(thisT, MCMCdraws, p, np, data, ydates, ...
                [], ...
    ...
    %% collect BETA moments
    BETAmedian(:,:,ndxT)       = squeeze(median(BETA_all,1));
    BETAmean(:,:,ndxT)         = squeeze(mean(BETA_all,1));
    BETAstdev(:,:,ndxT)        = squeeze(std(BETA_all,1,1));
    BETAquantiles(:,:,:,ndxT)  = permute(prctile(BETA_all,setQuantiles,1), [2 3 1]);
