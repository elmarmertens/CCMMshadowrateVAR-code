...
[PAI_all, PHI_all, invA_all, sqrtht_all, shadowrate_all, ...
    ] = mcmcVARshadowrateBlockHybrid(thisT, MCMCdraws, ...
    p, np, data, ydates, ...
    actualrateBlock, ...
...
save(sprintf('mcmcBlockHybrid-%s.mat', titlename), '-v7.3')
...
if doIRF
    for IRF1scale = irfSCALES
        ...
        for irfDate = irfDATES
            ...
            % prepare wrap
            titlename=sprintf('hybridshadowrate-%s-p%d-IRF1scale%d-jumpoff%s-irfDate%s', datalabel, p, IRF1scale, ...
...
function ydraws = simVARshadowrateBlockHybrid(N, fcstX0, ndxfcstY, ndxfcstActual, ndxfcstShadow,  cumcode, np, fcstA, fcstB, invA, nushocks, ELBbound, ndxYIELDS, irfHorizon, irfNdraws) %#ok<DEFNU>
