...
modellabel = 'FullyHybridShadowVsMissingRateSampling';
...
spmd(Nworker)
    ...
    [PAI_all, PHI_all, invA_all, SV_all, shadowrate_all, missingrate_all ...
        ] = mcmcVARhybridGibbs(thisT, MCMCdraws, p, np, data, ydates, ...
        [], ...
...
for n = 1 : Nshadowrates
    ...
    % ylim([-8 4])
    srlim = ylim;
    wrapthisfigure(thisfig, sprintf('fullyhybridshadowrate%d-vs-missingrate-%s%s', n, datalabel, ELBtag), wrap)
    ...
    xtickdates(thesedates)
    ylim(srlim)
    wrapthisfigure(thisfig, sprintf('fullyhybridshadowrate%d-vs-missingrate-MissingDataSampling-%s%s-step1', n, datalabel, ELBtag), wrap)
    ...
    wrapthisfigure(thisfig, sprintf('fullyhybridshadowrate%d-vs-missingrate-MissingDataSampling-%s%s', n, datalabel, ELBtag), wrap)
...
for n = 1  :  N
    ...
    wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-PAI-%s', datalabel, ELBtag, ncode{n}), wrap)
...
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-interceptPAI', datalabel, ELBtag), wrap)
...
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-allPAI', datalabel, ELBtag), wrap)

legend([hlag, hint, h45], 'Lag coefficients', 'Intercepts', '45 degrees', ...
    'box', 'off', 'location', 'northwest')
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-allPAI-WITHLEGEND', datalabel, ELBtag), wrap)

title(sprintf('R^2 = %4.2f', scatter.rsqr))
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-allPAI-withR2', datalabel, ELBtag), wrap)
...
h45 = plot(xlim, ylim, 'k--');
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-PAIlag', datalabel, ELBtag), wrap)

% legend([hlag, hint, h45], 'Lag coefficients', 'Intercepts', '45 degrees', ...
%     'box', 'off', 'location', 'northwest')
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-PAIlag-WITHLEGEND', datalabel, ELBtag), wrap)

title(sprintf('R^2 = %4.2f', scatter.rsqr))
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-PAIlag-withR2', datalabel, ELBtag), wrap)
...
set(gca, 'fontsize', 12)
view(-20,67)
wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-ELBPAIall', datalabel, ELBtag), wrap);
...
for n = 1 : N
    ...
    xtickdates(thesedates)
    wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-SVmid-%s', datalabel, ELBtag,  ncode{n}), wrap)
    legend([hShadow hMissing], 'Shadow-rate VAR', 'Missing-rate VAR', 'box', 'on', 'location', 'best')
    wrapthisfigure(thisfig, sprintf('fullyhybrid-%s%s-missing-vs-shadow-SVmid-%s-WITHLEGEND', datalabel, ELBtag, ncode{n}), wrap)
