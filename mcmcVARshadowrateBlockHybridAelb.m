function [PAI_all, PHI_all, invA_all, invAelb_all, sqrtht_all, shadowrate_all, missingrate_all, ...
    fcstYdraws, fcstYhat, ...
...
if hasELBdata
    ...
    VARatELB           = any(yNaN(:,elb.ndxS),2);
    if VARatELB(1)
        error houston
    end
    VARlaggedAtELB     = VARatELB([1, 1 : end-1]);
    ...
    elb.bound = ELBbound;
    elb.AtELB = VARlaggedAtELB(elbT0+1:end);
...
for ii = 2 : N
    ...
    OMEGA_A_inv(1:ii-1,1:ii-1,ii)              = precisionA * Iim1;
    OMEGA_A_inv(ii:thisNareg,ii:thisNareg,ii)  = precisionAelb * Iim1; % elb dummies
end;
...
while m < MCMCreps % using while, not for loop to allow going back in MCMC chain
    ...
    while stationary==0;
        ...
        PAI = CTAsysAswitching(Y,XX,N,K,T,A_,Aelb_,VARlaggedAtELB,sqrtht,iV,iVb_prior,PAI,rndStream);
    ...
    for ii = 2 : N
        ...
        theseRESID   = RESID(:,1 : ii - 1) ./ sqrtht(:,ii);
        X_spread_adj = [theseRESID theseRESID .* VARlaggedAtELB];
        ...
        alphadraw         = reshape(alphadraw, ii-1, 2);

        A_(ii,1:ii-1)    = - alphadraw(:,1)';
        Aelb_(ii,1:ii-1) = - sum(alphadraw,2)';
    ...
    RESIDtilde              = NaN(T,N);
    RESIDtilde(~VARlaggedAtELB,:) = RESID(~VARlaggedAtELB,:) * A_';
    RESIDtilde(VARlaggedAtELB,:)  = RESID(VARlaggedAtELB,:)  * Aelb_';
    ...
    if hasELBdata
        ...
        for tt = 1 : length(elb.AtELB)
            if elb.AtELB(tt)
                elb.B(elb.ndxY,:,tt)  = invAelb_;
            else
                elb.B(elb.ndxY,:,tt)  = invA_;
            end
        end
    ...
    if m > MCMCburnin
        ...
        if thiniter == MCMCthinstride
            ...
            invAelb_all(thisMCMCdraw,:,:)    = invAelb_;
            ...
            if doPredictiveDensity
                ...
                for nn = 1 : Ndraws
                    ...
                    for hh = 1 : fcstNhorizons
                        lastAtELB    = any(fcstX0(ndxfcstY(ndxYIELDS)) < ELBbound);
                        if lastAtELB
                            theseShocks = invAelb_ * nushocks(:,hh,nn);
                        else
                            theseShocks = invA_ * nushocks(:,hh,nn);
                        end
                        fcstXdraw    = fcstA * fcstX0 + fcstB * theseShocks;
                    ...
                    if doIRF1
                        ...
                        for hh = 1 : fcstNhorizons
                            lastAtELB    = any(fcstX0(ndxfcstY(ndxSHADOWRATE)) < ELBbound);
                            if lastAtELB
                                theseShocks = invAelb_ * nushocks(:,hh,nn);
                            else
                                theseShocks = invA_ * nushocks(:,hh,nn);
