...
for p = 12 % [12 6 3]
    ...
    titlename = sprintf('oosPairwiseEvaluationTablesELB500-%s-p%d', datalabel, p);
    initwrap
    ...
    models(m).resultlabel  = sprintf('standardVAR-ELB500-RATSbvarshrinkage-p%d', p);
    if p == 12
    ...
    m = m0 +  4;
    models(m).datalabel    = datalabel;
    models(m).resultlabel  = sprintf('ELBsampling-ELB500-RATSbvarshrinkage-p%d', p);
    if p == 12
    ...
    m = m0 +  6;
    models(m).datalabel    = datalabel;
    models(m).resultlabel  = sprintf('ELBblockhybrid-ELB500-RATSbvarshrinkage-p%d', p);
    if p == 12
    ...
    for m = 1 : length(MODELS)
        ...
        for sam = 1 : 4
            ...
            if doCharts
                tabname = sprintf('allinoneELB500Chart-%s-%s-vs-%s-%s.tex', models(m0).datalabel, ...
                    models(m0).shortlabel, models(m1).shortlabel, evaltxt);
            else
                tabname = sprintf('allinoneELB500-%s-%s-vs-%s-%s.tex', models(m0).datalabel, ...
                    models(m0).shortlabel, models(m1).shortlabel, evaltxt);
            end
...
if ~doCharts
    ...
    fprintf(fid, 'Note: Comparison of ``%s'''' (baseline, in denominator) against ``%s,'''' and assuming an ELB of 50 basis points. Values below 1 indicate improvement over baseline. \n', ...
        prettylabel0, prettylabel1);
    fprintf(fid, '%s \n', comparisonNote);
