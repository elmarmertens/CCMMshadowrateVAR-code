...
modeltype1           = 'BlockHybrid';
resultsdir1          = 'irfVXOEBP';

modeltype0           = 'Linear';
resultsdir0          = 'irfVXOEBP';
...
for dd = 1 : length(DATALABEL)
    ...
    for IRF1scale = irfSCALES
        ...
        for irfDate = irfDATES
            ...
            titlename=sprintf('%s-p%d-IRF1scale%d-jumpoff%s-irfDate%s-%s-vs-%s', datalabel, p, IRF1scale, ...
                datestr(jumpDate, 'yyyymmm'), datestr(irfDate, 'yyyymmm'), modeltype0, modeltype1);
            ...
            %% load IRF
            % load 0
            filename=sprintf('irf%s-%s-p%d-IRF1scale%d-jumpoff%s-irfDate%s', modeltype0, ...
                datalabel, p, IRF1scale, ...
                datestr(jumpDate, 'yyyymmm'), datestr(irfDate, 'yyyymmm'));
            irf0 = matfile(fullfile(resultsdir0, filename));

            % load 1
            filename=sprintf('irf%s-%s-p%d-IRF1scale%d-jumpoff%s-irfDate%s', modeltype1, ...
                datalabel, p, IRF1scale, ...
                datestr(jumpDate, 'yyyymmm'), datestr(irfDate, 'yyyymmm'));
            irf1 = matfile(fullfile(resultsdir1, filename));
            ...
            for n = 1 : N
                ...
                yline(0, 'k:')
                legend([h0, h1], modeltype0, modeltype1, 'location', 'best')
                ...
                sgtitle(sprintf('%s\n(shocks of size %d at %s)', Ylabels{n}, IRF1scale, datestr(irfDate, 'yyyy mmm')), 'FontSize', 18', 'FontWeight', 'bold')

                wrapthisfigure(thisfig, sprintf('IRF1plusminus-%s-%s-IRF1scale%d-%s-jumpoff%s-irfDate%s-%s-vs-%s', modeltype0, datalabel, IRF1scale, ncode{n}, ...
                    datestr(jumpDate, 'yyyymmm'), datestr(irfDate, 'yyyymmm'), modeltype0, modeltype1), wrap)
            end
